...
%  a round turbulent jet. A physical interpretation of the SPOD results is 
...
%   additional storage requirement by saving only a few modes at selected
%   frequencies.

opts.savefft    = true;             % save FFT blocks instead of keeping them in memory
...
%   default (Hamming) window of length 256 with 128 snapshots overlap is
...
for fi = [10 15 20]
    for mi = [1 2]
        ...
        xlabel('x'), ylabel('r'), title(['f=' num2str(f(fi),'%.4f') ', mode ' num2str(mi) ', \lambda=' num2str(L(fi,mi),'%.4g')])
        xlim([0 10]); ylim([0 2])
        count = count + 1;
    end
end
